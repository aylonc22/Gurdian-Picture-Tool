...
namespace Gurdian_picture_tool
{
    public partial class Form2 : SfForm
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //CompressImage(bmfull, saveFile + "\\Full Light" + bmName + " light.png", 80);
            //CompressImage(bm, saveFile + "\\Card Light" + bmName + " light.png", 80);
        ...
        private void pictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (getRatio(crpY, true) == e.Y || getRatio(crpY + height1, true) == e.Y)
                    this.Cursor = Cursors.SizeNS;
                else
                    if (getRatio(crpX) == e.X || getRatio(crpX + width1) == e.X)
        ...
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            if (getRatio(crpY, true) < e.Y && getRatio(crpY + height1, true) > e.Y && getRatio(crpX) < e.X && getRatio(crpX + width1) > e.X)
                shouldMove = true;
            if (getRatio(crpY, true) == e.Y)
                shouldTop = true;
            else
            if (getRatio(crpY + height1, true) == e.Y)
                shouldBot = true;
            else
            if (getRatio(crpX) == e.X)
                shouldLeft = true;
            else
            if (getRatio(crpX + width1) == e.X)
                shouldRight = true;
        }
        ...
        private void pictureBox2_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (getRatio(crpY2, true) == e.Y || getRatio(crpY2 + height2, true) == e.Y)
                    this.Cursor = Cursors.SizeNS;
                else
                    if (getRatio(crpX2) == e.X || getRatio(crpX2 + width2) == e.X)
                    this.Cursor = Cursors.SizeWE;
                else
        ...
        private void pictureBox2_MouseDown(object sender, MouseEventArgs e)
        {
            if (getRatio(crpY2, true) < e.Y && getRatio(crpY2 + height2, true) > e.Y && getRatio(crpX2) < e.X && getRatio(crpX2 + width2) > e.X)
                shouldMove2 = true;
            if (getRatio(crpY2, true) == e.Y)
                shouldTop2 = true;
            else
            if (getRatio(crpY2 + height2, true) == e.Y)
                shouldBot2 = true;
            else
            if (getRatio(crpX2) == e.X)
                shouldLeft2 = true;
            else
            if (getRatio(crpX2 + width2) == e.X)
                shouldRight2 = true;
        }
        ...
        private static void JsWrap(string input,string output,bool _temp = true)
        {
            ...
            startInfo.WorkingDirectory = AppDomain.CurrentDomain.BaseDirectory + "\\js";
            if (flag)
            {
